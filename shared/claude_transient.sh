...
# ⛔ SAFETY-LOCK[frame-break] — 안전 가드레일(운영자 260714). 이 함수는 모델의 콘텐츠 거절(영어 메타·비동의 성폭력 등 거절 벽)을 탐지해 대사 박제를 막는 축이다.
#   약화·삭제·감지 완화(임계 하향·패턴 제거) 금지 — LLM 세션 포함. 오탐(정상 대화 오인) 축소는 허용하나 탐지 범위 축소는 금지. 변경 = check_refs SAFETY-LOCK 게이트가 감지.
