...
_FWD_UNUSED = {
    ...
    # --brand/--brand-rgb = 메인 다크 글래스 전환(260704 V2)으로 배선 완료(바탕 글로우·로고 T·접속 버튼·네비 활성·--pin-ac 스왑) → 집합 제거.
    # ▼ 라이트 메인 팔레트(첨부2·운영자 승인분) = 같은 전환으로 일괄 미배선 — 라이트 뷰 후속용 어휘 보존(정의 삭제 대신 forward-unused 파킹·260704)
    '--accent-1', '--accent-1-rgb', '--accent-2-rgb', '--cream', '--sky', '--sky-rgb',
    '--m-danger', '--m-danger-rgb', '--m-fg', '--m-fg2', '--m-glass', '--m-glass2',
    '--m-line', '--m-line2', '--m-mut',
    '--pin-ac-rgb',   # --pin-ac 짝(rgba 채움용 어휘) — 커스텀 키패드(lk-key.hit) 폐지로 현재 미배선(260704)
    '--press-s',   # 눌림 사다리 s티어 — 하단 네비(.ynav button:active) 폐지로 현재 미배선. 사다리 어휘 보존(향후 소형 버튼용) = forward-declared(260704)
}
