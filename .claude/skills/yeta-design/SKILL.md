...
- **디자인 제안 = 플레이그라운드식이 디폴트**(확립본 = `docs/reports/260706_언락밝기_플레이그라운드.html` · 정본인덱스 4절 등재). 구성 5요소: ① 실물 재현 미리보기{폰 프레임 · `:root` 토큰 사본 + 정본 컴포넌트·커브 계승 · 현행 상태 = 비교 기준으로 함께 재현} ② 프리셋 3~4개(현행 포함 · 추천 ★ 표시) ③ 미세조정 슬라이더(축별 그룹 라벨) ④ 선택값 출력+복사 버튼{값마다 현행 병기 · 계승/갱신 후보 자동 표기 · 기틀 축 = ⚠ 딱지} ⑤ 재현 한계 각주(폴백·근사·실물 차이 명시). 자산 = base64 임베드(자기완결 1파일). 전달 = 레포 커밋(정본) + 채팅 파일 직접 첨부(원탭) · Artifact = 재열람·공유 보조.
