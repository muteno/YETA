...
| 브랜드 색(yeta·네온레몬) | `--brand #E7FF2E`·`--brand-rgb 231,255,46` | — | 채움 시 전경 = 잉크 `--m-fg`(대비 ≈16:1) · 근흑 위 텍스트 ≈17:1 | — | — | 로고·CTA·활성 강조·내 버블(스펙 = `구성도/01_페이지맵_yeta.html`~06) | — | ⚠️ 라이트 바탕 레몬 단독 텍스트 금지(≈1:1) · 의미색(`--danger/--arm`)·페르소나색 침범 금지 | viewer `:root`(운영자 260704 고정·이 레포 한정) · 값 = 플랫폼 기존 레몬 채택 · 현행 화면(버블 보라·메인 오렌지/코발트) 스왑은 구성도 승인 후 일괄 |
