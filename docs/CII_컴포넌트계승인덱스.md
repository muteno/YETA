...
- **디스플레이/테크 폰트(260704 운영자 승인)** = `--font-display`(LEMONMILK · 로컬 `viewer/fonts/` · YETA 로고=포스터 첨부2 정합·검증) · `--font-tech`(Chakra Petch · 로컬 `viewer/assets/fonts/` · 사이버펑크 HUD·PIN 숫자). 로그인 히어로 타이틀 = 애니 오버레이(라벨 페이드인 `ymLabIn` · write-on `ymWrite` · YETA 마스킹 스윕 `ymSweep` 5s · 백라이트/비네트 `ymBreathe`). 배경 = 캐릭터 전용 포스터(`shared/clean_hero.py`가 main.png 텍스트/화살표 제거 → `yeta-hero.webp`).
