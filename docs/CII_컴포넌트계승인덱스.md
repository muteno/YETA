...
| 에픽 등급 배지(yeta) | `.yepic`(생성 = `yEpicBadge(c)` · `c.grade==='epic'` 옵트인) | 인라인 pill | `--epic` 12% 플레이트+테 `.4`(= `.yprem` 선언 계승·색만 등급 퍼플) | — | ✦ 텍스트 "에픽" | 이름 옆(보이스 `.yprem` 앞 · 훅 5곳 병렬) | pad 1/8·`--r-pill` | title="에픽 등급" | index `yEpicBadge`(코어 — call.js 아님·음성 모듈과 독립) |
