...
| 브랜드 색(yeta·네온레몬) | `--brand #E7FF2E`·`--brand-rgb 231,255,46` | — | 채움 시 전경 = 잉크 `--m-fg`(대비 ≈16:1) · 근흑 위 텍스트 ≈17:1 | — | — | 워드마크 TA 블록·글로우 서클·내 버블·전송·PIN·네비 활성·공통 강조(스펙 = `구성도/01_페이지맵_yeta.html`~06) | — | ⚠️ 라이트 바탕 레몬 단독 텍스트 금지(≈1:1) · 의미색(`--danger/--arm`)·페르소나색 침범 금지 | viewer `:root`(운영자 260704 고정·이 레포 한정) · 값 = 플랫폼 기존 레몬 채택 · 260704 적용 완료(다크 온리 V2 · 라이트 V1 팔레트 폐지) |
