...
| PIN 입력칸(메인 게이트) | `.ym-card`(글래스 카드) + `.pin-slots`/`.pin-slot` + `.pin-in-h`(투명 입력) | 슬롯 12×18 | **dialog 글래스 정본 계승**(그라데 165deg·blur30 saturate1.6·선 rgba255,.13·inset 하이라이트·radius `--r-modal`) | 오답=`lkShake`+`--danger` | 없음 — 방금 자리=`--pin-ac`(=`--brand` 스왑) 숫자·이전/미입력=`--mut` 동그라미 | 메인 `.ym-pin`(pin-mode) | 카드 pad 26/22 | `inputmode=numeric`+`aria-label`·탭=OS 숫자키패드(커스텀 키패드 lk-pad 폐지) | index `#ymSlots`(업로드 데모 정본 이식·260704 V2) |
