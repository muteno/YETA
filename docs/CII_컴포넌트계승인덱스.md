...
| 채팅 버블(yeta) | `.yb.me`/`.yb.ai`(+`.sys`/`.fail`) · 캐릭터 = `.ymsg`(아바타+이름+버블) | me max-w 78%·msg 86% | me=`--bubble-me`(=`--accent` 네온라임·글자 `--yeta-bg` 근흑 · 운영자 260704)·ai=`--bubble-ai`(#1e2023) — :root 토큰 | — | *나레이션*=`i.yn` 이탤릭 | 꼬리쪽 모서리만 `--r-s` | radius `--r-l`·pad 10/13 | 캡션 `.yb-cap`=tabular | index `#yetadlg`(yRender·yAiMsg) |
...
| 메인 PIN 게이트(yeta) | `.ym-yeta`(글래스 레터 로고)+`.ym-field`(PIN 단독 입력)+`.ynav`(플로팅 네비 알약) | 로고 64px·PIN 폭 min(220px,70vw)·네비 min(92vw,430px)=nm-toast 계승 | 로고 = clip-text 글래스 그라데(흰 .92→.38·T=`--accent-rgb` .95→.45·그림자 nm-toast 계승) · PIN·네비 = 글래스 `rgba(255,255,255,.05/.06)`+`--line`·blur18 saturate1.3 | 네비 `--press-s`·오류 = lkShake+`--danger` | 잠금 lockdot = 라인 SVG | 로고+PIN 정중앙·설명글 0(운영자 260704) · 네비 하단 플로팅 | radius `--r-l`/`--r-pill`·PIN pad 13/16 | input `aria-label`+자동완성 4종 off·오류 `role=alert`·focus-within 링 = 챗 입력칸(.yeta-in textarea) 계승(`--accent` 보더) | index `#ymain`(V2.1 다크 글래스·PIN 타이핑·260704) |
