...
| 챗 좌상단 미터(yeta) | `.ymeter`(`#yetaMeter`) | 10px(광학 · raw 예외축)·tabular | 무도형 · `--mut` | — | — | `.yeta-stage` 절대 좌상단(헤더 아래) · `pointer-events:none`(기록용 상시·비간섭) | top 62+safe·left 14 | `aria-hidden` · 형식=`모델 --effort eff ┃ 누적 tok`(다이얼=YDIAL · 토큰=답장 턴 `tok{i,o}` 합 = 러너 claude_meter 실측) | index `yMeter`(운영자 260709 첨부3) |
| 전화 락 픽토(yeta) | `#yetaCallBtn .ycall-lock` | svg 11 | 수화기=`--mut`(미배선 `.nophone`) · **락=`--think` 터콰이즈**(운영자 260709 — 옛 동색 저투명이 '열쇠 꽂힌' 오독) · 그림자=drop-shadow(토큰 없는 축) | — | 자물쇠 SVG(`.ym-access` path 계승) | 수화기 정중앙 오버레이(inset:0 grid) | — | 미배선 캐릭터만 표시 · 방 전환 동기=`YCALL.sync`(yApply 훅) | viewer/call.js `initCallBtn`·`callBtnSync` |
