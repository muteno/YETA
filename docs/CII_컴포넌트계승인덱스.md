...
| 무전 마이크(yeta) | `#yetaMic`(`.ycall-mic`) | `--btn` · svg 21 | **픽토그램-온리 · 강조색 라임 `--accent`**(도형[글래스 원] 제거 · 운영자 260705) · 녹음=`--danger` 점멸 | `--press-m`(svg) | 마이크 SVG 단일 path | `.yeta-in` 좌측(모듈 주입) | — | `aria-label` 무전 · 녹음중 placeholder 교체 | viewer/call.js `initPtt`+`ensureCss`(스타일 로드 시 주입 — 옛 버그: `ensureDom`[콜 때만]에만 있어 마이크가 기본 버튼으로 뜨던 것 수정) |
