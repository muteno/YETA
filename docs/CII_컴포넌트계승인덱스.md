...
| 채팅 버블(yeta) | `.yb.me`/`.yb.ai`(+`.sys`/`.fail`) · 캐릭터 = `.ymsg`(아바타+이름+버블) | me max-w 78%·msg 86% | me=`--bubble-me`(=`--accent` 네온레몬 #d8ff3d·글자 `--yeta-bg` 근흑 · 운영자 260704)·ai=`--bubble-ai`(#1e2023) — :root 토큰 | — | *나레이션*=`i.yn` 이탤릭 | 꼬리쪽 모서리만 `--r-s` | radius `--r-l`·pad 10/13 | 캡션 `.yb-cap`=tabular | index `#yetadlg`(yRender·yAiMsg) |
