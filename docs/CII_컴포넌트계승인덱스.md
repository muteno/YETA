...
| 브랜드 알약 CTA | `.bpill` | h≈36(pad 9/18) | **투명 + `--brand` 1.5px 아웃라인 + 레몬 텍스트**(고스트 필 · 운영자 260704 레퍼런스) | `--press-l`(.95) | 텍스트 = **Pretendard**(`--fs-label`·`--fw-b` · LEMONMILK 금지=워드마크 전용)±이모지 표식 | 다크 표면 전용(라이트 위 금지) | `--r-pill` | `:focus-visible` 레몬 링 | index `.bpill` · 도입처 1호 = 캐릭터 리스트 🎲(`.yl-dice`) · P0 4메뉴 CTA 정본(대화 시작 등) |
| 채팅 버블(yeta) | `.yb.me`/`.yb.ai`(+`.sys`/`.fail`) · 캐릭터 = `.ymsg`(아바타+이름+버블) | me max-w 78%·msg 86% | me=`--bubble-me(=var(--brand) 네온레몬)`+잉크 글자(`--m-fg`)·ai=`--bubble-ai`(#1e2023) — :root 토큰(브랜드 적용 260704) | — | *나레이션*=`i.yn` 이탤릭 | 꼬리쪽 모서리만 `--r-s` | radius `--r-l`·pad 10/13 | 캡션 `.yb-cap`=tabular | index `#yetadlg`(yRender·yAiMsg) |
...
| 브랜드 색(yeta·네온레몬) | `--brand #CFFF40`·`--brand-rgb 207,255,64` | — | 채움 시 전경 = 잉크 `--m-fg`(대비 ≈16:1) · 근흑 위 텍스트 ≈17:1 | — | — | 워드마크 TA 블록·글로우 서클·내 버블·전송·PIN·네비 활성·공통 강조(스펙 = `구성도/01_페이지맵_yeta.html`~06) | — | ⚠️ 라이트 바탕 레몬 단독 텍스트 금지(≈1:1) · 의미색(`--danger/--arm`)·페르소나색 침범 금지 | viewer `:root`(운영자 260704 고정·이 레포 한정) · 값 = 플랫폼 기존 레몬 채택 · 260704 적용 완료(다크 온리 V2 · 라이트 V1 팔레트 폐지) |
