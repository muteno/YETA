...
| 스페셜 등급 훈장(yeta) | `.ygrade`+`.epic`/`.idol`(생성 = `yGradeBadge(c)` · `c.grade` 옵트인 — 시즌 특전 · 동렬·상호배타 · **보이스 `.yprem`은 별개 "기능 아이콘" 축**) | 인라인 pill | 베이스 = `.yprem` 선언 계승 + **양각 sheen**(`inset 0 1px 0 --glass-line`) · epic = `--epic` 12% 플레이트+테 `.4` · idol = 흰·실버(`--glass-2`+`--glass-line`+`--fg` — 신규 색 0) | — | **보석 젬 SVG**(외곽+하단 파셋 컷·drop-shadow 양각 — 파형과 실루엣 차별) + 등급명(에픽/IDOL) | 이름 옆(보이스 `.yprem` 앞 · 훅 5곳 병렬) | pad 1/8·`--r-pill` | title="스페셜 등급 — {등급}" | index `yGradeBadge`(코어 — call.js 아님·음성 모듈과 독립 · 운영자 260707 훈장/기능 분리) |
...
- **직업·변신 배지 `.yjob`/`.yjob.tq`**(운영자 260707 "이름 옆 직업/변신모드"): 플레이트 = `.yprem` 정본 계승(패딩 1px 8px·`--r-pill`·12% bg·40% border) · 메인 직업 = `--accent` 라임 강조 · 변신 모드 = **`--tq` 터콰이즈**(#2BFFD6 · 강조3 신설 — 변신 직업군 전용 축·CTA 아님) · 데이터 = roster `job`(전원)/`mode{nm,t[]}`(시간대 게이트 · KST ymSlot) · 훅 = `yJobBadge(c)` 이름 직후(등급·보이스 앞).
