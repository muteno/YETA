...
| 채팅 버블(yeta) | `.yb.me`/`.yb.ai`(+`.sys`/`.fail`) · 캐릭터 = `.ymsg`(아바타+이름+버블) | me max-w 78%·msg 86% | me=`--bubble-me`(제타 보라 #7c5cfc)·ai=`--bubble-ai`(#1e2023) — :root 토큰(260703 v2) | — | *나레이션*=`i.yn` 이탤릭 | 꼬리쪽 모서리만 `--r-s` | radius `--r-l`·pad 10/13 | 캡션 `.yb-cap`=tabular | index `#yetadlg`(yRender·yAiMsg) |
...
| 메인 사인인 게이트(yeta) | `.ym-field`(비밀번호 카드)+`.ym-go`(접속)+`.ynav`(플로팅 네비 알약) | 폭 min(92vw,430px)=nm-toast 계승 | 카드·버튼 = 글래스 `rgba(255,255,255,.05/.06)`+`--line` · 네비 = blur18 saturate1.3(nm-toast 글래스 계승) | 접속 `--press-l`·네비 `--press-s`·오류 = lkShake+`--danger` | 잠금 lockdot = 라인 SVG | body 세로중앙 좌정렬(스크린샷 사인인 결) · 네비 하단 플로팅 | radius `--r-l`/`--r-pill`·카드 pad 13/16 | input `aria-label`+자동완성 4종 off·오류 `role=alert`·focus-within 링 = 챗 입력칸(.yeta-in textarea) 계승 | index `#ymain`(V2 다크 사인인·비번 타이핑·260704) |
