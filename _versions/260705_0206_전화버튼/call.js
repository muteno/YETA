...
//   전화 요청 = {op:'ring'} · 실전화 = {op:'phone'}(Vapi 스캐폴드 — 이 모듈은 수신·무전만)
//
// [소비 규약 — 기틀 CLAUDE.md §🗺]
...
function closeDlg() { stopFx(); stopVoice(); cur = null; if (dlg && dlg.open) dlg.close(); }
function decline() { if (cur) markSeen(cur.ts); closeDlg(); }
async function accept() {
  if (!cur) return closeDlg();
  const call = cur;
  markSeen(call.ts); stopFx();
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initPtt); else initPtt();   // 무전 마이크 주입(defer 로드 = 보통 즉시)
