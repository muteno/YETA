...
try:
  ...
  local dead=0; printf '%s' "${1:-}" | grep -qiE '<<[[:space:]]*DEAD' && dead=1   # 사망 턴(운영자 260714 "마지막 말" 푸시) — OUT 원문은 finish 전이라 <<DEAD>> 잔존
  prev="$(printf '%s' "${1:-}" | python3 -c "
import sys,re
t=sys.stdin.read()
t=re.split(r'<<\s*(?:NOTE|MOOD|DEAD)', t, 1, flags=re.I)[0]   # 마커 이전 = 대사만(짧은 답장서 기억·무드·사망 마커 미리보기 누출 차단 · 260714)
t=re.sub(r'\*[^*]*\*','',t)                # 지문 제거 = 대사만(미리보기)
t=re.sub(r'\s+',' ',t).strip()
print((t[:70]+'…') if len(t)>70 else (t or '새 메시지'))")"
  local ttl="$nm"; [ "$dead" = 1 ] && ttl="${nm}의 마지막 말"   # 사망 = 유서처럼 도착(운영자 260714 한 수)
