...
// ── 헤더 수화기 버튼(모듈 주입 · 본체 무수정) — 보이스톡(브라우저 실시간·roster phone 배선 캐릭터) · 없으면 인앱 걸려오는 전화(op ring) ──
...
function initCallBtn() {
  ...
  b.addEventListener('click', async () => {
    ...
    if (p.phone) {   // 보이스톡 — 벨 연출 후 받기 탭에서 SDK 연결(카톡 보이스톡 페이스 · 운영자 목업 이식)
      open({ ts: Date.now(), persona: pid, text: '', voice: '', web: 1, assistant: p.phone });
      return;
    }
