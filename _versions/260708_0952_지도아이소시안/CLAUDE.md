...
7. **색 = yeta 브랜드 팔레트 v2**(260704 운영자 그린 전환): 강조1 브랜드 라임 `--accent:#CFFF40`·강조2 코발트 형광 `--cobalt:#3D6BFF`·뉴트럴 흰(`--fg`)/스포티파이블랙(`--bg:#121212`)·글래스모피즘(`--glass*`·`--blur-*`). 강조(CTA)는 **이 2색만**, 나머지 흰/근흑. **+ 스페셜 등급(훈장) 축**(운영자 260707 — 시즌 특전 · 보이스[기능 아이콘 `.yprem`]와 별개): 에픽 = 퍼플 형광 `--epic:#B026FF`(시즌 주인공 · 배지·감정배경 **전용** — CTA 금지 = 2색 제한 취지와 공존 · 기틀감사 APPROVE) · IDOL = 흰·실버(뉴트럴 `--fg`+글래스 계승 = 신규 색 0) · 등급 동렬·상호배타(`grade` 슬롯 1개) · 배지 = 보석 젬+양각(`.ygrade`). 옛 네온보라(`--bubble-me`)·라이트 코발트/오렌지(`--accent-1/2`·`--m-*`) 폐지 → 전체 다크·그린 통일(챗 버블 포함). 구조 토큰(반지름·간격·타이포·모션·눌림) 계승은 그대로.
