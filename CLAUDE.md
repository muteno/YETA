...
- 페르소나 = **랜덤 뽑기 + 🎲 재뽑기**(고정 선택 없음·화자만 교체·맥락 승계). 내 버블 = 네온라임(`--bubble-me`=`--accent` · 운영자 260704 확정). *나레이션* = 이탤릭.
...
7. **색은 yeta 팔레트**(네온라임 `--accent`·근흑) — 운영자 260704 네온 라임 확정(`--bubble-me`=`--accent` 합류 · 브랜드 교체 = 이 토큰만 · 라임 위 글자 = `--yeta-bg` 근흑). 구조 토큰(반지름·간격·타이포·모션·눌림)만 계승.
