...
## 🔒 절대명령 #1 (레포 최상위 · 모든 규칙에 우선) — 디자인 = 루트 토큰 「계승」 또는 「갱신」, 둘 뿐

> **이 조항이 이 문서·모든 규칙보다 우선한다. 위반 감지 = 즉시 중단하고 운영자에 확인.**

웹앱의 **모든 디자인 값**(색·크기·간격·반지름·blur·모션·눌림·그림자·컴포넌트 등 시각값 전부)은 예외 없이 둘 중 하나다:
1. **계승 (디폴트)** — `viewer/index.html` `:root` 루트 디자인 토큰을 `var()`로 가져와 쓴다.
2. **갱신** — 계승할 토큰이 없을 때만. **raw 값 임의 창작·인라인 하드코딩 절대 금지.**

**계승할 게 없으면 → 임의로 진행하지 말고 운영자에게 물어본다:**
- ① 토큰을 **갱신(신규 추가)**할지, ② 아니면 **무엇으로 계승**할지(가장 가까운 기존 토큰 후보를 제시) — **운영자 승인 후에만** 진행.
- 승인분만 즉시 기틀 편입: `:root` 토큰 추가 → `shared/build_design_mirror.py build`(거울) → `docs/CII_컴포넌트계승인덱스.md` 행 → `check_refs` baseline 사유.

= 아래 §🎨 디자인 계약의 **절대화**. 충돌 시 이 절이 최상위. (운영자 260704 제1 핵심 명령)
